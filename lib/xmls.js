...
const transformOptionals = (optionals) => {
  return _.mapValues(optionals, (value) => {
...
const getStartWorkflowSynchInnerSoap = (posId, transactionId, amount, currency, callbackUrl, signature, shopComment, optionals) => {
  const {
    languageCode = 'hu',
    nameNeeded = 'FALSE',
    cardPocketId,
  ...
  return `<?xml version="1.0" encoding="utf-8"?>
    <StartWorkflow>
      ...
      <Variables>
        ...
        ${cardPocketId ? `<isCardPocketId>${cardPocketId}</isCardPocketId>` : '<isCardPocketId/>'}
        <isCountryNeeded>${countryNeeded}</isCountryNeeded>
        <isCountyNeeded>${contyNeeded}</isCountyNeeded>
        <isSettlementNeeded>${settlementNeeded}</isSettlementNeeded>
        <isZipcodeNeeded>${zipcodeNeeded}</isZipcodeNeeded>
        <isStreetNeeded>FA${streetNeeded}LSE</isStreetNeeded>
        ...
        ${consumerRegistrationId ? `<isConsumerRegistrationID>${consumerRegistrationId}</isConsumerRegistrationID>` : '<isConsumerRegistrationID/>'}
      </Variables>
    </StartWorkflow>`;
}
