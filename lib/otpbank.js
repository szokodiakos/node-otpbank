...
class Otpbank {
  ...
  static _validateOptionals(optionals) {
    const validator = {
      languageCode: String,
      nameNeeded: Boolean,
      cardPocketId: String,
    ...
    const errors = _.map(optionals, (value, key) => {
      if (!validator[key]) {
        return `Parameter: ${key} is not valid`;
      }
      if (!(validator[key] === Boolean && _.isBoolean(value))) {
        return `Parameter: ${key} must be a boolean`;
      }
      if (!(validator[key] === String && _.isString(value))) {
    ...
    if (!_.isEmpty(errors)) {
      throw Error(`Validation errors occured: ${JSON.stringify(errors)}`);
    }
  }
  ...
  startWorkflowSynch(transactionId, callbackUrl, amount, currency, shopComment, optionals) {
    ...
    Otpbank._validateOptionals(optionals);
